...
const GroupItem = ({ item, width, navigation }) => {
    return (
        <TouchableOpacity
            ...
            <View>
                {item?.status == 'online' && <View style={{ width: 12, height: 12, backgroundColor: item?.status == 'online' ? colors.green : colors.red, borderWidth: 1, borderColor: '#fff', borderRadius: 13, position: 'absolute', zIndex: 1, left: -2 }} />}
                <Image source={
